...
RC BplusTreeIndex::create(const char *file_name, const IndexMeta &index_meta, std::vector<const FieldMeta *> &field_meta)
{
  if (inited_) {
  ...
  RC rc = index_handler_.create(file_name, field_meta[0]->type(), field_meta[0]->len());
  if (RC::SUCCESS != rc) {
...
{
  return index_handler_.delete_entry(record + field_meta_[0].offset(), rid);
}
